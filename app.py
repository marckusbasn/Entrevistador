...
# 5. DIRETRIZES ÉTICAS E DE SEGURANÇA
...
# A mensagem de abertura agora também é uma variável
...
if "chat" not in st.session_state:
    st.session_state.chat = None # Inicializa como None para indicar que o chat ainda não foi iniciado
    st.session_state.messages = []
    st.session_state.vignette_chosen = False # Estado para controlar a vinheta
    
    st.session_state.messages.append({"role": "model", "content": mensagem_abertura})
    st.session_state.messages.append({"role": "system", "content": orientacoes_completas})


# Exibe o histórico da conversa
for message in st.session_state.messages:
    if message["role"] != "system": # Não exibe a mensagem do sistema
...
if prompt := st.chat_input("Sua resposta...", key="chat_input"):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Pensando..."):
            ...
            if st.session_state.vignette_chosen == False:
                # O usuário acabou de responder à mensagem de abertura. Agora, apresente a vinheta.
                vinheta_escolhida = random.choice(vinhetas)
                st.session_state.messages.append({"role": "model", "content": vinheta_escolhida})
                st.session_state.vignette_chosen = True
                
                # Inicia o chat com a IA
                st.session_state.chat = genai.GenerativeModel('gemini-1.5-flash').start_chat()
                st.session_state.chat.history = [
                    {"role": "user", "parts": [orientacoes_completas]},
                    {"role": "model", "parts": [vinheta_escolhida]}
                ]

                # Agora, sim, a IA responde à primeira mensagem do usuário com a vinheta
                st.write(vinheta_escolhida)

            else:
                # Se a vinheta já foi escolhida, a conversa segue normalmente
