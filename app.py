...
def pagina_entrevistador():
    ...
    def classificar_intencao(prompt_utilizador):
        ...
        prompt_classificador = f"""Você é um classificador de intenções. A intenção pode ser 'ENTREVISTA' ou 'PESQUISA'. Classifique: "{prompt_utilizador}". Responda APENAS 'PESQUISA' ou 'ENTREVISTA'."""
        try:
    ...
    def formatar_para_nvivo(chat_history, participant_id):
        ...
        timestamp_inicio = st.session_state.start_time.astimezone(fuso_horario_br).strftime("%d-%m-%Y %H:%M")
        ...
        for msg in chat_history:
            ...
            timestamp = msg.get('timestamp', st.session_state.start_time).astimezone(fuso_horario_br).strftime('%H:%M:%S')
    ...
    if prompt := st.chat_input("Sua resposta...", key="chat_input", disabled=st.session_state.get('interview_over', False)):
        ...
        with st.chat_message("assistant"):
            ...
            # <<< O "MOTOR" COMPLETO DA ENTREVISTA ESTÁ RESTAURADO AQUI >>>
            try:
