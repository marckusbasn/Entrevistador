...
def pagina_entrevistador():
    ...
    def classificar_intencao(prompt_utilizador):
        """
        Usa a IA com um prompt melhorado (few-shot) para classificar a intenção.
        """
        ...
        try:
            ...
            # Adiciona uma verificação extra para garantir a resposta
            if "PESQUISA" in response.text:
                return "PESQUISA"
            return "ENTREVISTA"
        except Exception:
            # Em caso de qualquer erro, assume-se que é uma resposta da entrevista por segurança
            return "ENTREVISTA"

    # O resto das funções (responder_pergunta_pesquisa, etc.) e a lógica principal
    # permanecem exatamente as mesmas. O código completo está abaixo.
    pass

# ==============================================================================
# CÓDIGO COMPLETO PARA COPIAR E COLAR
# ==============================================================================
if 'pagina_entrevistador' in locals() and 'pagina_configuracao' in locals():
    # Definições completas das funções para garantir que o código seja autocontido
            ...
            with st.chat_message("assistant"):
                if st.session_state.model is None:
                    ...
                    if prompt.lower().strip() in negative_responses:
                        ...
                        if not st.session_state.get('transcript_saved'): save_transcript_to_github(st.session_state.messages)
                    else:
    ...
    if st.query_params.get("admin") == "true":
        pagina_configuracao_completa() # Chamar a definição completa da função
    else:
        pagina_entrevistador_completa()
