...
# <<< IMPORTANTE: COLOQUE AQUI O SEU LINK DO GOOGLE FORMS >>>
GOOGLE_FORM_LINK = "https://docs.google.com/forms/d/e/1FAIpQLSfwbE7jsSBd1vRgh0alaCIkeqAuoTC8w-c4E6M7XnRAS9y-1Q/viewform" 
# <<< IMPORTANTE: COLOQUE AQUI O NÚMERO DA SUA "ENTRY" DO CAMPO ID_Entrevista >>>
FORM_ID_ENTRY = "600654660" 

# --- Textos e Orientações da IA ---
# (O seu prompt completo e robusto permanece aqui, omitido por brevidade)
...
# (O restante do código, incluindo as funções e a lógica da aplicação, permanece o mesmo.
...
def save_transcript_to_github(chat_history, participant_id):
    ...
    except Exception as e: print(f"Erro ao salvar no GitHub: {e}"); return False
...
if st.session_state.stage == "welcome":
    ...
    # <<< TEXTO DE INSTRUÇÃO MELHORADO AQUI >>>
    st.write("Após submeter o questionário, a página de confirmação pedirá para você **voltar a este separador** do seu browser.")
    st.info("Ao regressar, clique no botão abaixo para começar a sua entrevista.")
    
    # <<< BOTÃO MELHORADO AQUI >>>
    if st.button("Passo 2: Iniciar Entrevista"):
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                if "<END_INTERVIEW>" in full_response_text:
                    st.session_state.stage = "thank_you" # Muda para o estágio de agradecimento
...
# ESTÁGIO 3: FIM DA ENTREVISTA
    ...
    st.success("A entrevista foi encerrada e a sua transcrição foi salva com sucesso.")
    st.info("Obrigado pela sua participação. Já pode fechar esta janela.")
    # Exibe a conversa final, mas desativa o input
    for message in st.session_state.messages:
        with st.chat_message(message["role"]):
            st.write(message["content"])
