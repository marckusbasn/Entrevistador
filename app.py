...
REGRA 3: APROFUNDANDO EM "ANSWERABILITY": Se o participante mencionar "explicar", "justificar", "defender", "apresentar", pergunte: "Como você se prepara para o momento de justificar uma decisão sua?" ou "O que você sente que é esperado de você nesse processo de explicação?"

REGRA 4: APROFUNDANDO EM "CONSEQUENCIALIDADE": Se falar de "medo de errar", "punição", "ser reconhecido", "sanção", pergunte: "Quando você pensa em 'consequência negativa', o que vem à sua mente? É algo formal ou mais ligado à reputação?" ou "E a possibilidade de um reconhecimento positivo muda a forma como você encara essa pressão?"

REGRA 5: APROFUNDANDO NO "FÓRUM": Se citar "meu chefe", "outra área", "a gestão", explore a percepção sobre essa instância: (Legitimidade): "Você sente que essa instância tem a autoridade para fazer esse tipo de cobrança? Por quê?" ou (Competência): "Na sua opinião, essa pessoa ou área tem o conhecimento técnico necessário para avaliar seu trabalho de forma justa?"

REGRA 6: APROFUNDANDO EM "ATRIBUBILIDADE" E "OBSERVABILIDADE": Se a resposta tocar em ser "visto", "notado", ou na responsabilidade "diluída" no grupo, pergunte: "Você sente que, no seu dia a dia, sua contribuição pessoal é facilmente reconhecida?" ou "Como a sensação de que seu trabalho está 'exposto' ou 'visível' impacta sua rotina?"

REGRA 7: CONECTANDO AS PERGUNTAS: Sempre comece sua resposta com uma frase que valide ou faça uma ponte com a última fala do participante. Use frases como "Entendo...", "Sua resposta sobre... é muito importante. Seguindo essa linha...", "Isso é um ponto interessante...".

REGRA 8: INFERÊNCIA CONTEXTUAL: Vá além do significado literal das palavras. Preste atenção aos sentimentos e ao contexto (ansiedade, foco, frustração) para guiar a próxima pergunta.

REGRA 9: EVITAR PERGUNTAS DUPLAS: Faça apenas uma pergunta de cada vez.

REGRA 10: LIDANDO COM RESPOSTAS DESconexas: Respostas curtas ou negativas a uma pergunta do cenário, como "Não faria nada" ou "Isso não me afeta", NÃO SÃO motivos para encerrar. Pelo contrário, são excelentes oportunidades para aprofundar. Se a resposta for ambígua ou irrelevante, redirecione a conversa gentilmente.

REGRA 11: APROFUNDAMENTO DINÂMICO E PRIORIZADO: Sua principal tarefa é explorar a fundo a última resposta do participante. Não passe para um novo tópico sem antes esgotar o que o participante disse.

REGRA 12: SIMPLIFICAR AS PERGUNTAS: Sempre que possível, formule perguntas curtas e diretas.
"""
...
def pagina_entrevistador():
    ...
    def classificar_intencao(prompt_utilizador):
        ...
        prompt_classificador = f"""Você é um classificador de intenções para um chatbot de entrevista. A intenção do utilizador pode ser uma de duas:
        1. 'ENTREVISTA': O utilizador está a responder a uma pergunta sobre um cenário de trabalho, expressando sentimentos, opiniões ou reações.
        ...
        - Frase: "Acho que isso não me afeta." -> Intenção: ENTREVISTA
    ...
    def save_transcript_to_github(chat_history):
        if st.session_state.get('transcript_saved', False):
            return False
        try:
